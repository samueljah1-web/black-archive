...
async function webSearch(q) {
  const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1500,tools:[{type:"web_search_20250305",name:"web_search"}],messages:[{role:"user",content:`Search for decolonial and Afrocentric scholarly sources about: "${q}". Summarise 4–6 key findings with sources, dates, and references. Focus on African and Black diasporal perspectives.`}]})});
  const d=await r.json();
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.96)",zIndex:3000,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",background:"#111"}}>
        {/* Inline map image viewer with zoom */}
        <div style={{flex:1,overflow:"auto",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <img
            src={map.thumb}
            alt={map.title}
            style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain",borderRadius:4,boxShadow:"0 4px 32px rgba(0,0,0,0.8)"}}
...
function AllScreen({T, globalQuery, activeTopic, setTab, scrapedBooks, scrapedImages, scrapedLinks, onScrapeAll, scraping, scrapeProgress}) {
  ...
  const allBooks = useMemo(()=>[...scrapedBooks,...(activeTopic?[]:[])],[scrapedBooks]);
  ...
  useEffect(()=>{
    ...
    setAiLoading(true);
    setAiResults(null);
    webSearch(globalQuery).then(r=>{setAiResults(r);setAiLoading(false);}).catch(()=>setAiLoading(false));
  },[globalQuery]);
...
function BooksScreen({T,activeTopic,globalQuery,scrapedBooks,onScrapeBooks,scraping}) {
  ...
  const manualSearch=async()=>{if(!q.trim())return;setSearching(true);setManualRes([]);
    try{const r=await fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(q)}&limit=10&fields=key,title,author_name,first_publish_year,subject,ia,cover_i,edition_count`);const d=await r.json();setManualRes((d.docs||[]).filter(b=>b.title).map(b=>({id:b.key,title:b.title,author:(b.author_name||[]).join(", ")||"Unknown",year:b.first_publish_year||"—",subjects:(b.subject||[]).slice(0,3),iaId:b.ia?.[0]||null,coverId:b.cover_i||null,editions:b.edition_count||1,source:"Open Library",readUrl:b.ia?.[0]?`https://archive.org/details/${b.ia[0]}`:`https://openlibrary.org${b.key}`,downloadUrl:b.ia?.[0]?`https://archive.org/download/${b.ia[0]}/${b.ia[0]}.pdf`:null})));}catch{}
    setSearching(false);};
...
function ImagesScreen({T,activeTopic,globalQuery,scrapedImages,onScrapeImages,scraping}) {
  ...
  const handleUpload=e=>{Array.from(e.target.files||[]).forEach(file=>{const r=new FileReader();r.onload=ev=>setUserImages(u=>[...u,{id:`u${Date.now()}${file.name}`,title:file.name,src:ev.target.result,source:"Uploaded",description:"",topics:[],valid:true}]);r.readAsDataURL(file);});};

  // Merge + filter, deduplicate, validate
  const allImages = useMemo(()=>{
    const base=[...CURATED_IMAGES,...scrapedImages,...userImages];
    const seen=new Set();
    return base.filter(i=>{if(!i.src||!i.src.startsWith("http")&&!i.src.startsWith("data:")) return false;if(seen.has(i.id)) return false;seen.add(i.id);return true;});
  },[scrapedImages,userImages]);

  const filtered = useMemo(()=>{
    let imgs=[...allImages];
    if(activeTopic) imgs=imgs.filter(i=>i.topics?.includes(activeTopic)||!i.topics?.length);
    if(globalQuery?.trim()){const q=globalQuery.toLowerCase();imgs=imgs.filter(i=>`${i.title} ${i.description} ${(i.topics||[]).join(" ")} ${i.source}`.toLowerCase().includes(q)||q.split(" ").some(w=>w.length>2&&`${i.title} ${i.description}`.toLowerCase().includes(w)));}
    return imgs;
  },[allImages,activeTopic,globalQuery]);
...
function LinksScreen({T,activeTopic,globalQuery,scrapedLinks,onScrapeLinks,scraping}) {
  ...
  const allLinks = useMemo(()=>{
    const base=[...CURATED_LINKS,...scrapedLinks];
    const seen=new Set();
    return base.filter(l=>{if(!l.url||!l.title||!l.url.startsWith("http")) return false;if(seen.has(l.id)) return false;seen.add(l.id);return true;});
  },[scrapedLinks]);

  const filtered = useMemo(()=>{
    let ls=[...allLinks];
    if(globalQuery?.trim()){const q=globalQuery.toLowerCase();ls=ls.filter(l=>`${l.title} ${l.description} ${l.category||""} ${l.topic||""}`.toLowerCase().includes(q)||q.split(" ").some(w=>w.length>2&&`${l.title} ${l.description}`.toLowerCase().includes(w)));}
    return ls;
  },[allLinks,globalQuery]);
...
function MapsScreen({T,activeTopic,globalQuery}) {
  ...
  const filtered = useMemo(()=>{
    let maps=[...OLD_MAPS];
    if(activeTopic) maps=maps.filter(m=>m.topics?.includes(activeTopic)||!m.topics?.length);
    if(globalQuery?.trim()){const q=globalQuery.toLowerCase();maps=maps.filter(m=>`${m.title} ${m.description} ${(m.topics||[]).join(" ")} ${m.source} ${m.year}`.toLowerCase().includes(q)||q.split(" ").some(w=>w.length>2&&`${m.title} ${m.description}`.toLowerCase().includes(w)));}
    return maps;
  },[activeTopic,globalQuery]);
...
function VideosScreen({T,activeTopic,globalQuery}) {
  ...
  const filtered = useMemo(()=>{
    let vs=[...CURATED_VIDEOS];
    if(activeTopic) vs=vs.filter(v=>v.topics?.includes(activeTopic));
    if(globalQuery?.trim()){const q=globalQuery.toLowerCase();vs=vs.filter(v=>`${v.title} ${v.description} ${(v.topics||[]).join(" ")} ${v.channel}`.toLowerCase().includes(q)||q.split(" ").some(w=>w.length>2&&`${v.title} ${v.description}`.toLowerCase().includes(w)));}
    return vs;
  },[activeTopic,globalQuery]);
...
function AuthorsScreen({T,activeTopic,globalQuery}) {
  ...
  const filtered = useMemo(()=>{
    let as=[...KEY_AUTHORS];
    if(activeTopic) as=as.filter(a=>a.topics?.includes(activeTopic));
    if(globalQuery?.trim()){const q=globalQuery.toLowerCase();as=as.filter(a=>`${a.name} ${a.field} ${a.legacy} ${(a.topics||[]).join(" ")} ${(a.works||[]).join(" ")}`.toLowerCase().includes(q));}
    return as;
  },[activeTopic,globalQuery]);
...
export default function App() {
  ...
  const [globalQuery, setGlobalQuery] = useState(""); // committed search
  ...
  const saveBooks = useCallback(books => {
    setScrapedBooks(books);
    localStorage.setItem("ba9-books", JSON.stringify(books));
  }, []);
  const saveImages = useCallback(images => {
    setScrapedImages(images);
    localStorage.setItem("ba9-images", JSON.stringify(images));
  }, []);
  const saveLinks = useCallback(links => {
    setScrapedLinks(links);
    localStorage.setItem("ba9-links", JSON.stringify(links));
  }, []);
  ...
  return (
    <div style={{minHeight:"100vh",background:TH.bg0,color:TH.txt1,fontFamily:F.body,fontSize:14,display:"flex",transition:"background 0.3s,color 0.3s"}}>
      ...
      {/* ── SIDEBAR — stable component outside App render ── */}
      <Sidebar
        T={TH} tab={tab} setTab={setTab}
      ...
      <div style={{flex:1,marginLeft:SB_W,display:"flex",flexDirection:"column",minWidth:0,minHeight:"100vh",transition:"margin-left 0.2s ease"}}>
        ...
        <div style={{background:TH.isDark?"#0a0906":"#ffffff",borderBottom:`1px solid ${TH.border}`,padding:"0 20px",display:"flex",alignItems:"center",gap:14,height:56,flexShrink:0,position:"sticky",top:0,zIndex:50,boxShadow:TH.shadow}}>
          ...
          {/* Centre — Google-style search bar */}
          <div style={{flex:1,maxWidth:560,margin:"0 auto"}}>
            <div style={{display:"flex",alignItems:"center",background:TH.isDark?"#161410":"#ffffff",border:`1.5px solid ${globalQuery?TH.ndY:TH.border}`,borderRadius:28,padding:"0 6px 0 16px",boxShadow:globalQuery?`0 0 0 2px ${TH.ndY}20`:TH.shadow,transition:"all 0.2s"}}>
              <input
                style={{flex:1,background:"transparent",border:"none",color:TH.txt0,padding:"10px 10px",fontSize:14,fontFamily:F.body,outline:"none",minWidth:0}}
                placeholder="Search the archive… e.g. colonization in the Kongo"
                value={searchInput}
                onChange={e=>setSearchInput(e.target.value)}
                onKeyDown={e=>{if(e.key==="Enter")submitSearch();if(e.key==="Escape"){setSearchInput("");setGlobalQuery("");}}}
              />
              {searchInput&&(
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          {/* Hero — only on home when no search */}
          {tab==="all"&&!globalQuery&&(
